...
# def get_embds():
#     model_path = "/datas/huggingface/Qwen1.5-7B-Chat"
#
...
class MatrixFactorization(nn.Module):
    ...
    def __init__(self, config, *args, **kwargs) -> None:
        ...
        item_path = {
            "movie":'../../step2_train_collab/output/movie/item_embeds.pt',
            "book":"../../step2_train_collab/output/book/item_embeds.pt",
        }
        data_type = "book"
        pretrained_weights = torch.load(item_path[data_type])
    ...
    #     # 选择前 k 个最大特征值对应的特征向量
    #     k = 1024  # 你想要的主成分数
    #     _, indices = torch.sort(eigenvalues, descending=True)  # 直接对 1D 的特征值排序
    #     top_k_eigenvectors = eigenvectors[:, indices[:k]]  # 选择前k个特征向量
    #
    #     # 将原始数据投影到选定的主成分空间中
    #     X_reduced = torch.mm(X_centered, top_k_eigenvectors)  # 输出降维后的数据
    #
    #     return X_reduced
...
if __name__=='__main__':
    ...
    data_dirs = {
        ...
        "book": "/root/autodl-tmp/Projects/CoLLM-QWen2/data/Amazon-Book/processed/data/"
    }
